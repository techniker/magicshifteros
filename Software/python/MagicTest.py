...
baudrate = 9600 #Baudrate goes here
...
def issueUpload(device, data, sector):	
	ser = None
	try:    
		...
		dataLen = len(data)		
		headerString = array.array('B', [sector, dataLen>>8, dataLen%256]).tostring()
		print dataLen
...
if __name__ == '__main__':
	if (len(sys.argv) >= 4 and sys.argv[1] == "up"):
		if (len(sys.argv) == 5):
			device = sys.argv[4]
		upload(sys.argv[2], int(sys.argv[3]))
