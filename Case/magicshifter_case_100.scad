...
dxfFileName = "magicshifter_case_100.dxf";
...
// bottom part (the one the pcb is in facing with the LEDs down and the 


*union()
{
	translate([0,0*32,0]) render() mirror([0,1,0]) topPart();
	translate([0,1*32,0]) render() mirror([0,1,0]) topPart();
	translate([0,2*32,0]) render() mirror([0,1,0]) topPart();
	translate([0,3*32,0]) render() mirror([0,1,0]) topPart();
}

*union()
{
	translate([0,0*32,0]) render() bottomPart();
	translate([0,1*32,0]) render() bottomPart();
	translate([0,2*32,0]) render() bottomPart();
	translate([0,3*32,0]) render() mirror([0,1,0]) topPart();
	
	translate([-20,0,0]) rotate([0,0,90]) render() mirror([0,1,0]) topPart();
	
	translate([-20-32,0,0]) rotate([0,0,90]) render() mirror([0,1,0]) topPart();
}



*rotate([0,0,90]) for (i = [0:1])
{
	translate([i*64, 0,0]) completeCase();
}

translate([65, -20,0]) rotate([0,0,90]) completeCase();

// button cover (TODO: assembled view of this)
if (generateButtonCover) extrudeLayer("buttoncover", buttonH);
...
module completeCase()
{
	...
	rotate ([0,0,90])
	{
		if (generateBottomPart) bottomPart();
		
		// top part (the lid with the pillars); top part is mirrored to make drawing the dxf easier
		if (generateTopPart)
		{
			if (forPrint) translate([0,32,0]) mirror([0,1,0]) topPart();
			if (!forPrint) translate([0,0,totalH]) mirror([0,0,1]) topPart();
		}
	}
}
...
module topPart()
{
	difference() {
		...
		for (pos = microButtonPositions) {
			translate([pos[0], pos[1], -1]) {
				cylinder(r= 1.35, h = 3*totalH, center=true);
			}
		}
